...
from SpeakLearnBot.database.base import async_session_maker
from SpeakLearnBot.database.models import (
    FileModel,
...
async def seed_watch_videos(session: AsyncSession) -> None:
    from SpeakLearnBot.data.watch_video_data import VIDEO_LIST
...
async def seed_positive_feedbacks(session: AsyncSession) -> None:
    from SpeakLearnBot.data.positive_feedback import POSITIVE_FEEDBACKS
...
async def seed_russian_case_questions(session: AsyncSession) -> None:
    from SpeakLearnBot.data import russian_cases_quiz_data
...
async def seed_russian_tutor_prompts(session: AsyncSession) -> None:
    from SpeakLearnBot.data.russian_tutor import SYSTEM_PROMPT
...
async def seed_sing_along_songs(session: AsyncSession) -> None:
    from SpeakLearnBot.data.sing_along_data import SING_ALONG_SONGS
...
async def seed_speech_practice(session: AsyncSession) -> None:
    from SpeakLearnBot.data.speech_practice_quiz_data import SPEECH_PRACTICE_DATA
...
async def seed_translate_words(session: AsyncSession) -> None:
    from SpeakLearnBot.data.translate_word_quiz import WORD_DICTIONARIES
...
async def seed_verb_aspect_questions(session: AsyncSession) -> None:
    from SpeakLearnBot.data.verb_aspect_quiz import VERB_ASPECT_QUESTIONS
...
async def seed_verb_tense_questions(session: AsyncSession) -> None:
    from SpeakLearnBot.data.verb_tense_quiz import VERB_TENSE_QUESTIONS
